			...
			<div className="sticky top-0 z-50 max-w-5xl mx-auto px-8">
				<nav className="h-14 flex items-center justify-between bg-white/80 dark:bg-neutral-950/80 backdrop-blur-sm border-b border-neutral-200 dark:border-neutral-800">
					...
					<div className="flex items-center gap-1">
						...
						<a href="/benchmarks" className="px-3 py-1.5 text-sm text-black dark:text-white">
							Benchmarks
						</a>
						<a
							href="/playground"
			...
			<div className="max-w-5xl mx-auto px-8 py-16">
				<section className="pb-16 border-b border-neutral-200 dark:border-neutral-800">
					...
					<p className="text-lg text-neutral-500 dark:text-neutral-400 max-w-2xl mb-8">
						Performance comparisons against popular link preview libraries. All data sourced from
						npm and bundlephobia.
					</p>
				...
				<section className="py-16 border-b border-neutral-200 dark:border-neutral-800">
					<p className="text-xs font-mono text-neutral-400 tracking-widest mb-8">
						BUNDLE SIZE (GZIPPED)
					</p>
					<div className="space-y-6">
						{libraries.map((lib) => (
							<div key={lib.name} className="flex items-center gap-4">
								<span
									className={`text-sm w-44 font-mono ${lib.name === "openlink" ? "text-black dark:text-white font-medium" : "text-neutral-400"}`}
								>
									{lib.name}
								</span>
								<Bar value={lib.sizeBytes} max={maxSize} highlight={lib.name === "openlink"} />
								<span
									className={`text-sm w-20 text-right font-mono ${lib.name === "openlink" ? "text-black dark:text-white font-medium" : "text-neutral-400"}`}
								>
									{lib.size}
								</span>
							</div>
				...
				<section className="py-16 border-b border-neutral-200 dark:border-neutral-800">
					...
					<p className="text-neutral-500 dark:text-neutral-400 mb-8 max-w-2xl">
						Edge runtime support requires no Node.js-specific APIs like fs, path, or http. Libraries
						using Cheerio or similar DOM parsers typically don't work on edge runtimes.
					</p>
						...
						<table className="w-full text-sm">
							...
							<tbody>
								{libraries.map((lib) => (
									...
									>
										<td
											className={`py-3 font-mono ${lib.name === "openlink" ? "font-medium" : "text-neutral-500"}`}
										>
											{lib.name}
										</td>
				...
				<section className="py-16 border-b border-neutral-200 dark:border-neutral-800">
					...
					<div className="space-y-4 text-neutral-500 dark:text-neutral-400 leading-relaxed max-w-2xl">
						<p>
							Bundle sizes are approximate gzipped sizes from bundlephobia.com. Actual sizes may
							vary based on tree-shaking and bundler configuration.
						</p>
						<p>
							Parse time measures only HTML parsing and metadata extraction, excluding network
							request time. Each library parses the same pre-fetched HTML content.
						</p>
						<p>
							Edge runtime compatibility is determined by checking for Node.js-specific API usage.
							Libraries relying on Cheerio, jsdom, or similar DOM implementations typically don't
							work on edge runtimes.
						</p>
						<p>
							All data is sourced from npm and the respective package documentation. Run your own
							benchmarks for production decisions.
						</p>
					</div>
