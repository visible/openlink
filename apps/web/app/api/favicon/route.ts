...
export async function GET(request: NextRequest) {
  ...
  try {
    const result = await cached(
      cacheKey,
      async () => fetchBestFavicon(domain, theme),
      { ttl, fresh }
    )
...
async function fetchBestFavicon(domain: string, theme: "light" | "dark" | null): Promise<FaviconResult> {
...
function parseIconsFromHtml(html: string, domain: string, theme: "light" | "dark" | null): { url: string; priority: number; source: string }[] {
  const icons: { url: string; priority: number; source: string }[] = []
...
function parseLinkTag(tag: string, base: string, theme: "light" | "dark" | null): { url: string; priority: number; source: string } | null {
  ...
  const hasLightScheme = media.includes("prefers-color-scheme: light") || media.includes("prefers-color-scheme:light")
  const hasDarkScheme = media.includes("prefers-color-scheme: dark") || media.includes("prefers-color-scheme:dark")
...
async function fetchFromManifest(manifestUrl: string): Promise<FaviconResult> {
  try {
    ...
    const icons = manifest.icons as { src: string; sizes?: string; type?: string; purpose?: string }[] | undefined
...
async function fetchWithTimeout(url: string, timeout: number, asResponse?: true): Promise<Response | null>
async function fetchWithTimeout(url: string, timeout: number, asResponse?: false): Promise<string | null>
async function fetchWithTimeout(url: string, timeout: number, asResponse = false): Promise<Response | string | null> {
